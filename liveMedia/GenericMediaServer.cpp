...
int GenericMediaServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort) {
	...
	// ALLOW_RTSP_SERVER_PORT_REUSE is for backwards-compatibility #####
		...
		if (!increaseSendBufferTo(env, ourSocket, 50 * 1024)) break; // 50K
...
void GenericMediaServer::incomingConnectionHandlerOnSocket(int serverSocket) {
	...
	printf("-------------RTSP client socket %d------------- \r\n", clientSocket);

	if (clientSocket < 0) {
		int err = envir().getErrno();
		if (err != EWOULDBLOCK) { // 用于非阻塞模式，不需要重新读或者写
	...
	(void)createNewClientConnection(clientSocket, clientAddr); // 创建一个连接
}

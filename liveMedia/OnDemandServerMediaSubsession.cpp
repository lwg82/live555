...
OnDemandServerMediaSubsession::sdpLines() {
	if (fSDPLines == NULL) {
		...
		// subsession (as a unicast stream单播流).  To do so, we first create
		// dummy虚拟的 (unused) source and "RTPSink" objects,
...
::getStreamParameters(unsigned clientSessionId,
	...
	if (fLastStreamToken != NULL && fReuseFirstSource) { // 是否重用 streamToken, 如果重用，意味着多个客户端使用同一个source和sink,数据源相同

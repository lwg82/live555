...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
	...
	do {
		...
		if (parseSucceeded) {
			...
			// DESCRIBE
			//--------------------------------
			// 创建了ServerMediaSession和ServerMediaSubSession
			else if (strcmp(cmdName, "DESCRIBE") == 0) {
				handleCmd_DESCRIBE(urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);
			}
			//--------------------------------
			// SETUP 同时创建了 Session 对象
			//--------------------------------
				...
				if (!requestIncludedSessionId) {
					...
					if (authenticationOK("SETUP", urlTotalSuffix, (char const*)fRequestBuffer)) {
						// 创建 RTSPServer::RTSPClientSession
						clientSession
...
::handleCmd_withinSession(RTSPServer::RTSPClientConnection* ourClientConnection,
	...
	// - an aggregated operation, if "urlSuffix" is the session (stream) name, 聚合操作，如果“urlSuffix”是会话（流）名称，
	...
	if (fOurServerMediaSession == NULL) { // There wasn't a previous SETUP!
		...
		// Aggregated operation (聚合操作第一种情况)
		subsession = NULL;
	}
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
	...
	// Parse the client's "Scale:" header, if any: 倍速
	float scale;

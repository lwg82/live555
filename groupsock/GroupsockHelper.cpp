...
int setupStreamSocket(UsageEnvironment& env,
  ...
  // Windoze doesn't properly handle SO_REUSEPORT
	...
	if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
#endif
		MAKE_SOCKADDR_IN(name, ReceivingInterfaceAddr, port.num());  // name ???
		if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
			char tmpBuffer[100];
			sprintf(tmpBuffer, "bind() error (port number: %d): ",
				ntohs(port.num()));
...
static unsigned increaseBufferTo(UsageEnvironment& env, int bufOptName,
	...
	unsigned curSize = getBufferSize(env, bufOptName, socket); // 获取发送缓冲区大小
...
unsigned increaseSendBufferTo(UsageEnvironment& env,
	int socket, unsigned requestedSize) {
	return increaseBufferTo(env, SO_SNDBUF, socket, requestedSize); // 增加发送缓冲区大小
}
